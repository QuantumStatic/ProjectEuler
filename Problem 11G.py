...
def main():
    ...
    def left_and_right(grid):
        for x in range(20):
            for y in range(17):
                yield (grid[y+20*x] * grid[20*x+y+1] * grid[20*x+y+2] * grid[20*x+y+3])

    def up_and_down(grid):
        for x in range(20):
            for y in range(17):
                yield (grid[20*y+x] * grid[20*y+x+20] * grid[20*y+x+40] * grid[20*y+x+60])

    def main_diagonal(grid):
        for x in range(17):
            for y in range(17):
                yield (grid[20*x+y] * grid[20*x+y+21] * grid[20*x+y+42] * grid[20*x+y+63])

    def sub_diagonal(grid):
        for x in range(17):
            for y in range(3, 20):
